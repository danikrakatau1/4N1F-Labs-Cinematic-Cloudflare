...
- Package Key is `4N1F_` + 12 hex characters.
- Preview ID is `p_` + 32 hex characters.
...
The package publisher is responsible for storing the immutable snapshot and returning a Package Key. The public browser must never receive publisher secrets, Cloudflare API tokens, service-role credentials, or KV write credentials.
...
Creating or revising an experiment changes only package-store data. It must not require:

- a new static HTML route;
- a new special Preview ID constant;
- a Worker source edit;
- a Vercel deployment;
...
if (packageKey === SOME_EXPERIMENT_KEY) { ... }
```
