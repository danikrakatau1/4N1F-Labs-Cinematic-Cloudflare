...
try {
  ...
  await assert.doesNotReject(async () => {
    ...
    await frame.locator('#proof').waitFor({ state: 'visible', timeout: 15_000 });
  });
  ...
  assert.equal(await clean.locator('#__4n1f_box').count(), 0, 'Editor overlay leaked into Clean Preview');
  assert.equal(await clean.locator('[class*="__4n1f_"]').count(), 0, 'Editor classes leaked into Clean Preview');
  ...
  assert.ok(!zipHtml.includes('__4n1f_selected') && !zipHtml.includes('__4n1f_box'), 'Editor instrumentation leaked into ZIP');
